...
export async function apiCall<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
  ...
  if (Platform.OS !== "web") {
    ...
    console.log("[API] Authenticated request prepared", {
    ...
    console.log("[API] Web request prepared", { method: options.method || "GET" });
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error("[API] Request returned an error response", { status: response.status });
...
export async function exchangeOAuthCode(
  ...
  console.log("[API] OAuth code exchange started");
...
export async function getMe(): Promise<{
  ...
  try {
    ...
    console.error("[API] User lookup failed");
    return null;
  }
}
...
export async function establishSession(token: string): Promise<boolean> {
  try {
    console.log("[API] Establishing web session");
    const baseUrl = getApiBaseUrl();
    ...
    if (!response.ok) {
      console.error("[API] Web session establishment failed", { status: response.status });
      return false;
    }

    console.log("[API] Web session established");
    return true;
  } catch (error) {
    console.error("[API] Web session establishment failed");
    return false;
  }
}
